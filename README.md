...
版本号以 **`voiceink/version.py`** 中的 `__version__` 为准（当前 **1.4.1**）；安装包文件名、Inno 元数据与 Windows 下 `VoiceInk.exe` 属性均与之同步。
...
- **主窗口**（侧栏：历史 / 通用 / 引擎 / 润色 / 关于）收纳设置与历史；听写只留一条薄 **听写条**。托盘与听写条共用 模型载入中 / 正在听 / 正在识别 / 润色中 / 已复制。托盘 → **打开 VoiceInk**（Windows **双击**托盘打开主窗口）；配置保存在 `~/.voiceink/config.json`
...
- 双击标题栏最大化 / 还原，拖动右下角调整窗口；历史页中间分隔条可调整阅读宽度。
- 历史支持 **Ctrl / Shift 多选**、复制、导出与删除撤销；超过 50 场可继续加载旧会话。搜索与选中会话在刷新后保留。
...
安装 **[Releases](https://github.com/zyjhandsome/VoiceInk/releases)** 中的 **`VoiceInk-Setup-1.4.1.exe`**（约 500 MB）。无安装包见 [从源码打包](#从源码打包)。
