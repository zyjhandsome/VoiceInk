...
版本号以 **`voiceink/version.py`** 中的 `__version__` 为准（当前 **1.3.8**）；安装包文件名、Inno 元数据与 Windows 下 `VoiceInk.exe` 属性均与之同步。
...
安装 **`dist/VoiceInk-Setup-1.3.8.exe`**（Git LFS，约 498 MB；克隆后 `git lfs pull`）。无安装包见 [从源码打包](#从源码打包)。
