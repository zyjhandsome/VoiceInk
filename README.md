...
- **已下载 ≠ 已载入**：每次冷启动须载入内存（FireRedASR2 约 **10–40 秒**），等空间岛「已就绪」后再用
...
- **空间岛**浮窗 + 托盘与胶囊使用 模型载入中 / 正在听 / 正在识别 / 润色中 / 已复制：默认是顶部胶囊，点击展开实时出字并可打开设置 / 历史；配置保存在 `~/.voiceink/config.json`
...
1. 等顶部空间岛 **「模型载入中」** 消失；持续模式就绪后**不会**再弹「待开始」，请看托盘提示
2. 托盘 → **设置**（Windows **双击**托盘会唤醒空间岛；菜单仍可打开设置）→ **测试声音**；确认触发方式
...
> **快捷键无反应？** 确认不在「模型载入中」；持续转写需按住约 0.30 秒。若与输入法冲突，改用 **Alt+Space** 并保存设置。
...
**Q: 选好模型了，为什么启动还要等很久？**  
A: **下载/选好** ≠ **载入内存**；FireRedASR2 冷启动约 10–40 秒。等空间岛「已就绪」再按热键。

**Q: 托盘已就绪，空间岛还在加载？**  
A: 正常应同步；以空间岛为准，加载完成前勿按热键。
